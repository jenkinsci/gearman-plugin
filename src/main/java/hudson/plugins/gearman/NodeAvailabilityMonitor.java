...
public class NodeAvailabilityMonitor implements AvailabilityMonitor {
    ...
    public boolean canTake(Queue.BuildableItem item)
    {
        ...
        logger.debug("AvailabilityMonitor canTake request for " + item);

        NodeParametersAction param = item.getAction(NodeParametersAction.class);
        if (param != null) {
            if (expectedUUID != null &&
                expectedUUID.equalsIgnoreCase(param.getUuid())) {
                logger.debug("AvailabilityMonitor canTake item matched expected UUID " + expectedUUID);
                return true;
            } else {
        ...
        if (workerHoldingLock != null) {
            logger.debug("AvailabilityMonitor canTake hold by " + workerHoldingLock);
        }
